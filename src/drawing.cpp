...
int main() 
{
	string listPath = "G:/VOT/list.txt";
	String resultRootPath = "G:/VOT/";
	String gtRootPath = "G:/VOT/";
	String drawRootPath = "G:/VOT/_drawing/";
	...
	while (!listFile.eof())
	{
		...
		string gtPath = gtRootPath + sequenceName + "/groundtruth.txt";
		cout << gtPath << endl;
...
//			cout << PreRate[i] << endl;
		...
		{
			line(img, drawpoint[i], drawpoint[i + 1], Scalar(0, 0, 255), 2, 8);//连线
		}
		for (int i = 0; i < drawpoint2.size() - 1; i++)
		{
			line(img2, drawpoint2[i], drawpoint2[i + 1], Scalar(0, 0, 255), 2, 8);//连线
		}
		...
		putText(img, sequenceName+" precision rate", Point(axis_width*0.5, 1.5*height_shift + axis_height), 0, 1, Scalar(0, 0, 0));
		putText(img2, sequenceName+ " success rate", Point(axis_width*0.5, 1.5*height_shift + axis_height), 0, 1, Scalar(0, 0, 0));
		//imshow("precision", img);
		//imshow("success", img2);
		imwrite(drawRootPath + sequenceName + "_precision.jpg", img);
		imwrite(drawRootPath + sequenceName + "_success.jpg", img2);
		waitKey(3000);
	}
